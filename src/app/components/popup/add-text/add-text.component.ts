...
export class AddTextComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.fieldDisplay.type === 20 || this.fieldDisplay.type === 90 || this.fieldDisplay.type === 80) {
      const objSearch = {
    ...
    if (this.data.hosho.TypeF === 30 || this.data.hosho.TypeF === 35) {
  ...
  saveData() {
    ...
    else if (this.data.hosho.TypeF === 30 || this.data.hosho.TypeF === 35) {
    ...
    else if (this.data.hosho.TypeF === 20) {
    ...
    else if (this.data.hosho.TypeF === 3) {
    ...
    else if (this.data.hosho.TypeF === 90) {
      this.data.hosho.ColumnVal = this.mainFormControl.value;
      for (let i = 0; i < this.lsData.length; i++) {
        let item = this.lsData[i];
  ...
  changeEvent() {
    ...
    if (this.data.hosho.TypeF === 60) {
      const dialogCommon = this.dialog.open(SelectItemCommonComponent, {
        width: '400px',
        data: { popupTitle: '保険商品名選択', category: 'INSURANCY-TYPE', company_cd: this.data.hosho.company_cd },
        panelClass: ['remodal-wrapper', 'remodal-is-opened', 'custom-modal']
      });
      dialogCommon.afterClosed().subscribe(result => {
  ...
  checkRequired() {
    if (this.data.option.isShukeiyaku) {
      if (
        ...
        (
          this.data.hosho.TypeF !== 80 || (this.data.hosho.TypeF === 80 && (typeof this.noneFormControl.value === 'undefined' || this.noneFormControl.value === null || this.noneFormControl.value.toString().trim() === ''))
        )
      ) {
        return true;
      }
