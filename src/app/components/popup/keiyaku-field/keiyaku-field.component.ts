...
export class KeiyakuFieldComponent implements OnInit {
  ...
  ngOnChanges(changes) {
    if (this.fieldDisplay.type === 20 || this.fieldDisplay.type === 90 || this.fieldDisplay.type === 80 || this.fieldDisplay.type === 100) {
      const objSearch = {
    ...
    if (this.fieldDisplay.type === 10) {
      this.convertToMoney();
    }
    if (this.fieldDisplay.name === "Hosho" && this.fieldDisplay.type === 40) {
      this.httpService.post(API_URLS.getFamily, { login_id: localStorage.getItem('login_id') }).subscribe(res => {
        if (res.code === RESULT_CODE.success) {
